...
const postShopNotices = async ({
  shopId,
  hourlyPay,
  startsAt,
  workhour,
  description,
}: PostShopNoticesRequest): Promise<PostShopNoticesResponse> => {
  const response = await instance.post(`/shops/${shopId}/notices`, { hourlyPay, startsAt, workhour, description });
