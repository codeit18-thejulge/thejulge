...
const SearchInput = forwardRef<HTMLInputElement, SearchInputProps>(({placeholder, onSearch, ...props }, ref) => {
  ...
  return (
    <div className="order-3 flex w-full items-center gap-8 rounded-10 bg-gray-10 p-8 tablet:order-2 tablet:max-w-450 tablet:gap-10 tablet:p-10">
      ...
      <input
        ...
        className={cn("h-20 w-full bg-inherit text-12-regular text-black outline-none tablet:text-14-regular", placeholder === "" ? "placeholder-gray-40":"placeholder-black")}
        {...props}
      />
    </div>
  );
